...
const badgeStyles = {
  New: "bg-plum text-white border-plum",
  ...
  Sale: "bg-accent text-white border-accent",
...
export default function ProductCard({ product, index = 0 }) {
  ...
  return (
    <article
      className="group flex h-full min-w-0 flex-col overflow-hidden rounded-2xl border border-line bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg animate-fadeUp"
      style={{ animationDelay: `${(index % 4) * 70}ms` }}
      data-testid={`product-card-${product.slug}`}
    >
      <div
        className="relative aspect-[4/3] overflow-hidden bg-cream hover-zoom"
      ...
      >
        ...
        <button
          ...
          className="absolute right-3 top-3 flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-plum backdrop-blur transition-colors hover:bg-plum hover:text-white"
        ...
        <div className="mt-auto pt-3">
          {soldOut ? (
            ...
            <button
              ...
              className="flex h-11 w-full items-center justify-center gap-2 rounded-full bg-plum px-4 text-xs font-semibold uppercase tracking-widest text-white transition-colors hover:bg-plum-wine"
