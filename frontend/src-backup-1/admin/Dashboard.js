...
export default function Dashboard() {
  ...
  const [recent, setRecent] = useState({ items: [], pages: 1, total: 0 });
  ...
  useEffect(()=>{
    adminApi.get(`/orders?status=${encodeURIComponent(recentStatus)}&payment=${encodeURIComponent(recentPayment)}&sort=${recentSort}&q=${encodeURIComponent(recentQ)}&page=${recentPage}&page_size=8`)
      .then(({data})=>setRecent(data)).catch(()=>{});
  }, [recentQ,recentStatus,recentPayment,recentSort,recentPage]);
  ...
  useEffect(()=>{ setRecentPage(1); }, [recentQ,recentStatus,recentPayment,recentSort]);
  ...
  return <div>
    ...
    <KpiCards cards={cards.map(([label,value,Icon,sub])=>({label,value,icon:Icon,sub}))} />
    ...
    <div className="bg-white rounded-xl border border-gray-200 p-5 mt-6">
      <div className="flex items-center justify-between gap-3 mb-4"><div><h3 className="font-semibold">Recent Orders</h3><p className="text-xs text-gray-400">Click any row for the complete order record.</p></div></div>
      <DataTable
        testid="dashboard-orders" rows={recent.items} page={recentPage} pages={recent.pages} setPage={setRecentPage}
        q={recentQ} setQ={setRecentQ} searchPlaceholder="Search order / customer…"
        filters={[
          { key:"status", label:"All statuses", value:recentStatus, onChange:setRecentStatus, options:["Pending","Confirmed","Processing","Packed","Shipped","Out for Delivery","Delivered","Cancelled","Returned","Refunded","Failed"] },
          { key:"payment", label:"All payments", value:recentPayment, onChange:setRecentPayment, options:["created","paid","cod_pending","cod_confirmed","failed"] },
          { key:"sort", label:"Newest", value:recentSort, onChange:setRecentSort, options:[{value:"newest",label:"Newest"},{value:"oldest",label:"Oldest"},{value:"total_desc",label:"Highest value"},{value:"total_asc",label:"Lowest value"}] },
        ]}
        onRowClick={(o)=>setOpenOrder(o.order_number)}
        columns={[
          ...
          {key:"total",label:"Total",align:"right",render:o=><span className="font-semibold">{inr(o.pricing?.total)}</span>},
