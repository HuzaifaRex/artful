...
export default function ProductCard({ product, index = 0 }) {
  ...
  return (
    <div className="group animate-fadeUp border border-line bg-white" style={{ animationDelay: `${(index % 4) * 70}ms` }} data-testid={`product-card-${product.slug}`}>
      <div className="relative overflow-hidden bg-white aspect-[4/5] hover-zoom" onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}>
      ...
      <div className="p-4 text-center">
        ...
        {soldOut ? (
          <div className="mt-4 border border-line text-ink-muted text-[11px] uppercase tracking-widest2 py-2.5">Sold Out</div>
        ) : (
          <button onClick={() => addToCart(product, 1)} className="w-full mt-4 bg-plum text-white text-[11px] uppercase tracking-widest2 py-3 flex items-center justify-center gap-2 hover:bg-plum-wine transition-colors" data-testid={`quick-add-${product.slug}`}>
