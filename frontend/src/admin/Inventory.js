...
export default function Inventory() {
  ...
  const load = useCallback(async () => { setLoading(true); try {
    const [s,a,r,m,sup,pos,f,c] = await Promise.all([
      ...
      adminApi.get(`/inventory/movements?page=${movementPage}&page_size=50`), adminApi.get("/inventory/suppliers"), adminApi.get("/inventory/purchase-orders?limit=50"), adminApi.get(`/inventory/forecast?days=${days}`), adminApi.get("/categories")
