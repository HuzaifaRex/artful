...
async def _send_order_whatsapp(order, template_name, body_params, language_code=None):
    ...
    try:
        ...
        if not result.get("sent"):
            print(f"[whatsapp] order notification not sent for {order.get('order_number')}: {result.get('error') or result.get('message') or result}")
        return result
...
async def _finalize_paid_order(order, payment_id=None, method="razorpay"):
    ...
    if customer and customer.get("phone"):
        ...
        confirmation_params = await ig.build_order_confirmation_params(order_for_message)
